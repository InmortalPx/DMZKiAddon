...
public class ChargeEffects {
    ...
    private static final int CBEAM_INTERVAL = 14;
    ...
    public static void onChargeTick(int chargeTicks, int maxCharge,
        ...
        if (type == AttackType.FINAL_EXPLOSION) {
            spawnFinalExplosionAura(player, colorR, colorG, colorB, progress, chargeTicks);
        } else {
    ...
    private static void spawnFinalExplosionAura(Player player, float r, float g, float b, float progress, int chargeTicks) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.level == null) return;

        int auraCount = 2 + (int)(progress * 4);
        for (int i = 0; i < auraCount; i++) {
            double yOff = -0.1 + Math.random() * 2.5;
            double xOff = (Math.random() - 0.5) * 0.6 * progress;
            double zOff = (Math.random() - 0.5) * 0.6 * progress;
            ...
            if (auraP instanceof AuraParticle ap) {
                ap.resize(0.5f + progress * 2.0f);
            }
        }

        if (Math.random() < 0.7f) {
            double spread = 0.5 + progress * 0.6;
            mc.level.addParticle(
                    new DustParticleOptions(new Vector3f(r, g, b), 0.8f + progress * 1.0f),
        ...
        if (progress > 0.3f && player.level().getGameTime() % 2 == 0) {
            int ringCount = 6;
            long time = player.level().getGameTime();
            for (int i = 0; i < ringCount; i++) {
                double angle = (Math.PI * 2.0 / ringCount) * i + time * 0.12;
                ...
                mc.level.addParticle(
                        ...
                        -Math.sin(angle) * 0.06, 0.01, Math.cos(angle) * 0.06);
            }
        }

        if (chargeTicks % 10 == 0 && progress > 0.5f) {
            player.level().playSound(
                    null,
                    player.getX(), player.getY(), player.getZ(),
                    ModSounds.FINALFLASH_CHARGE.get(), SoundSource.PLAYERS,
                    0.3f + progress * 0.4f, 0.8f);
        }
    }
    ...
    private static void spawnAuraColumn(Player player,
        ...
        int auraCount = 1 + (int)(progress * 2);
        for (int i = 0; i < auraCount; i++) {
            double yOff = -0.1 + Math.random() * 2.5;
    ...
    private static void playChargeSound(Player player, float progress,
        ...
        SoundEvent sound = switch (type) {
            case KAMEHAMEHA, MASENKO, MAKANKOSAPPO,
                 ...
                 BIG_BANG, SPIRIT_BOMB -> getCbeam((chargeTicks / interval) % 7);
            case GALICK_GUN -> getFbeam((chargeTicks / interval) % 5);
            case FINAL_FLASH -> {
                if (chargeTicks < interval) yield ModSounds.FINALFLASH_CHARGE.get();
                yield getFbeam((chargeTicks / interval) % 5);
            }
            case FINAL_EXPLOSION -> ModSounds.FINALFLASH_CHARGE.get();
    ...
    public static void spawnPointPressureAura(Player player, float auraProgress) {
        ...
        final float rCore = 0.75f, gCore = 0.2f, bCore = 1.0f;
        ...
        for (int i = 0; i < auraCount; i++) {
            ...
            if (auraP instanceof AuraParticle ap) {
                ap.resize(0.25f + auraProgress * 0.9f);
            }
        }
        ...
        long time = player.level().getGameTime();
        for (int i = 0; i < ringCount; i++) {
            double angle = (Math.PI * 2.0 / ringCount) * i + time * 0.18;
