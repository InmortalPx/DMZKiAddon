...
public class KiAddonCommand {
    ...
    private static final Map<String, String> ATTACK_MAP = Map.ofEntries(
            ...
            Map.entry("time_skip",         "addon_time_skip"),
            Map.entry("point_pressure",    "addon_point_pressure"),
            Map.entry("final_explosion",   "addon_final_explosion"),
            Map.entry("kikoho",            "addon_kikoho"),
            Map.entry("neo_kikoho",        "addon_neo_kikoho")
    );
    ...
    private static final SuggestionProvider<CommandSourceStack> MASTER_SUGGESTIONS =
            (ctx, builder) -> SharedSuggestionProvider.suggest(
                    List.of("vegeta", "piccolo", "frieza", "tenshinhan", "hit"), builder
            );
    ...
    private static int executeLocate(CommandContext<CommandSourceStack> ctx, String master) {
        ...
        BlockPos pos = switch (master.toLowerCase()) {
            ...
            case "tenshinhan"  -> MasterSpawnManager.getTenshinhanPos(overworld);
            case "hit"         -> MasterSpawnManager.getHitPos(overworld);
            default -> null;
        };
